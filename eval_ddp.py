...
def parse_args():
    ...
    # Get local_rank from environment variables (if exists)
...
def init_distributed_mode(args):
    ...
    if local_rank != -1:
        # When starting with torchrun, these environment variables are automatically set
...
def build_model(args):
    ...
    model.set_generation_config(temperature=0, max_new_tokens=128)
...
def main(args):
    ...
    # Only the main process outputs log information
    if rank == 0:
        print("Initializing distributed environment...")
        print(f"World size: {world_size}, Rank: {rank}, Local rank: {local_rank}")
    
    if rank == 0:
        temp_dir = tempfile.mkdtemp(dir="/root/tmp")
    ...
    if args.task == "all":
        task_filenames = [
            ...
            for f in os.listdir(args.anns_dir)
            if f.endswith(".json")
        ]
    else:
    ...
    for task_filename in task_filenames:
        ...
        with open(os.path.join(args.anns_dir, dataset), 'r') as f:
                ...
                for lang in languages:
                    for task_instance in task_data:
                        ...
                        if lang == "cn":
                            ...
                            task_instance["prompt_to_evaluate"] = task_instance["instruction_zh"]
                        elif lang == "en":
    ...
    # Get tasks to be handled by the current process
    ...
    # Result summary: each process saves its own result to a temporary file, and then the main process merges them
    ...
    if rank == 0:
        ...
        logging.info(f"Evaluation of VenusBench-GD finished. Total samples: {len(all_results)}")
