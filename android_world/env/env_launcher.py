...
def _get_env(
    console_port: int, adb_path: str, grpc_port: int, adb_server_port: int, device_name: str
) -> interface.AsyncEnv:
  """Creates an AsyncEnv by connecting to an existing Android environment."""
  controller = android_world_controller.get_controller(
      console_port, adb_path, grpc_port, adb_server_port, device_name
  )
...
def load_and_setup_env(
  ...
  """
  env = _get_env(console_port, adb_path, grpc_port, adb_server_port, device_name)
