...
# yaml_path = os.path.normpath(os.path.join(current_dir, '../../venus_benchmark_settings.yaml'))
# # 读取 YAML 文件
...
def get_controller(
  ...
  config = config_classes.AndroidEnvConfig(
      ...
      simulator=config_classes.EmulatorConfig(
          ...
          adb_controller=config_classes.AdbControllerConfig(adb_path=adb_path, adb_server_port=adb_server_port, device_name=device_name),
      ),
  )
  android_env_instance = loader.load(config)
  logging.info('Setting up AndroidWorldController.')
  if device_name:
    return AndroidWorldController(android_env_instance, a11y_method=A11yMethod.UIAUTOMATOR)
  else:
    return AndroidWorldController(android_env_instance, a11y_method=A11yMethod.A11Y_FORWARDER_APP)
