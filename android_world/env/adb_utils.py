...
_PATTERN_TO_ACTIVITY = immutabledict.immutabledict({
    ...
    'simple draw pro|draw': 'com.simplemobiletools.draw.pro/com.simplemobiletools.draw.pro.activities.MainActivity',
    'pro expense|pro expense app|expense': (
        'com.arduia.expense/com.arduia.expense.ui.MainActivity'
    ),
    'broccoli|broccoli app|broccoli recipe app|recipe app|recipe': (
    ...
    'vlc|vlc app|vlc player': 'org.videolan.vlc/org.videolan.vlc.gui.MainActivity',
    'retro music|retro|retro player': (
        'code.name.monkey.retromusic/.activities.MainActivity'
    ),
    'calcyou|calcyou app': 'net.youapps.calcyou/.MainActivity',
    'pomodoro|tomato': 'org.nsh07.pomodoro/.MainActivity',
    'fitbook|fitbook app': 'com.presley.fit_book/.MainActivity',
    'zipxtract|zipxtract app|zip extract': 'com.wirelessalien.zipxtract/.FragmentActivity',
    'Medilog|medilog app|medilog|MediLog': 'com.zell_mbc.medilog/.MainActivity',
    'Calculator|calculator|calculator app': 'com.simplemobiletools.calculator/com.simplemobiletools.calculator.activities.MainActivity',
})
...
def push_file_to_emulator(
    ...
    """
    用 adb push 把服务器上的文件/目录拷贝到模拟器（或真机）文件系统。

    Args:
        local_path: 服务器端源路径（文件或目录，必须存在）。
        remote_path: Android 端目标路径（目录需以 / 结尾，文件需包含文件名）。
        env: android_world 环境句柄。
        timeout_sec: adb push 的超时时间。

    Raises:
        FileNotFoundError: 如果本地文件不存在。
        RuntimeError: adb push 失败时抛出。
    """
    if not os.path.exists(local_path):
        raise FileNotFoundError(f'Local file/dir not found: {local_path}')

    # # 通过 env 拿到 adb 命令前缀
    # device_name = get_device_name_from_env(env)
    # if not device_name:
    #     raise RuntimeError('Unable to determine device serial from env.')

    # 构造 adb push 命令
    adb_args = ['push', local_path, remote_path]

    # 通过 issue_generic_request 执行
    response = issue_generic_request(
        adb_args,
        env,
        timeout_sec=timeout_sec,
    ...
    # 检查返回
    if 'status: OK' in str(response.status):  # adb_pb2.AdbResponse.Status.OK == 0
        raise RuntimeError(
            f'adb push failed: {response.generic.output.decode()}'
        )

    print(f'✅ {local_path} --> {remote_path} ok')
...
def extract_package_name(activity: str) -> str:
  ...
  Args:
    activity: Activity string in format "package/activity" or None.
    
  Returns:
    The package name, or None if activity is None.
  """
  if activity is None:
    return None
...
def change_orientation(
  ...
  # issue_generic_request('adb shell settings put system user_rotation 1', env=env)
...
def notify_android(title, content):
    subprocess.run([
...
def show_system_notification(
  ...
  adb_pb2.AdbResponse
      执行结果。
  """
  # 对 title 和 message 做简单的 shell 转义
  def _escape_shell(s: str) -> str:
    return s.replace('"', '\\"')

  escaped_title = _escape_shell(title)
  escaped_msg = _escape_shell(message)

  adb_args = [
      'shell', 'service', 'call', 'notification',
      '1',               # 1 = INotificationManager.enqueueNotificationWithTag
      'i32', '1',        # flags = FLAG_AUTO_CANCEL
      's16', escaped_title,
      's16', escaped_msg,
      's16', 'com.android.systemui'  # 占位包名
  ]

  response = issue_generic_request(adb_args, env, timeout_sec)

  if response.status != adb_pb2.AdbResponse.Status.OK:
    logging.error('Failed to show system notification: %s', response.generic.output.decode())
...
def text_emulator(
  ...
  """Simulate an incoming text message using Android system service call.

  This method uses the Android SMS service directly via 'service call isms'
  which is more reliable than 'emu sms send' for remote devices.
  ...
  # Replace single quotes with '\'' sequence to properly escape them in shell
  escaped_message = message.replace("'", "'\\''")
  
  # Construct the service call command
  # service call isms 5: calls method 5 of the SMS service (likely sendText)
  # i32 0: slot ID (0 for first SIM)
  # s16 "service": service name
  # s16 "null": null parameter
  # s16 "phone": destination phone number
  # s16 "null": null parameter 
  # s16 "message": SMS message content
  # s16 "null": null parameter
  # s16 "null": null parameter
  # i32 1: flag parameter
  # i32 0: additional flag
  service_call_cmd = (
      f'service call isms 5 '
      f'i32 1 '
      f's16 "com.android.mms.service" '
      f's16 "null" '
      f's16 "{escaped_phone_number}" '
      f's16 "null" '
      f's16 "{escaped_message}" '
      f's16 "null" '
      f's16 "null" '
      f'i32 0 '
      f'i64 0'
  )
  adb_args = ['shell', service_call_cmd]
  response = issue_generic_request(adb_args, env, timeout_sec)
  return response


def text_emulator_by_emu(
    env: env_interface.AndroidEnvInterface,
    phone_number: str,
    message: str,
    timeout_sec: float = _DEFAULT_TIMEOUT_SECS,
) -> adb_pb2.AdbResponse:
  """Simulate an incoming text message using Android emulator."""
  phone_number = re.sub(r'[^0-9+]', '', phone_number)
  adb_args = ['emu', 'sms', 'send', phone_number, message]
  return issue_generic_request(adb_args, env, timeout_sec)

def text_emulator_service_reliable(
    env: env_interface.AndroidEnvInterface,
    phone_number: str,
    message: str,
    timeout_sec: float = _DEFAULT_TIMEOUT_SECS,
  ...
  """Simulate an incoming text message using Android system service call with retry logic.

  This method combines the service call approach with automatic retries and fallback
  to the original emulator method if needed.
  ...
  for attempt in range(max_retries):
    try:
      response = text_emulator_service(env, phone_number, message, timeout_sec)
      if response.status == adb_pb2.AdbResponse.Status.OK:
        logging.info(f'SMS sent successfully using service call (attempt {attempt + 1})')
        return response
      else:
        logging.warning(f'Service call attempt {attempt + 1} failed with status: {response.status}')
    except Exception as e:
      last_exception = e
      logging.warning(f'Service call attempt {attempt + 1} failed with exception: {e}')
      if attempt < max_retries - 1:
        time.sleep(1)  # Wait before retry
  
  # If service call fails, fallback to original emulator method
  logging.info('Service call method failed, falling back to emulator method')
  try:
    response = text_emulator(env, phone_number, message, timeout_sec)
    logging.info('SMS sent successfully using fallback emulator method')
    return response
  except Exception as e:
    logging.error(f'Both service call and emulator methods failed. Last exception: {e}')
    if last_exception:
...
def get_device_info(env: env_interface.AndroidEnvInterface) -> dict:
    """获取设备的完整配置信息
    ...
    """
    try:
        config = env._coordinator._simulator._config
        return {
            'device_name': config.adb_controller.device_name,
            'adb_path': config.adb_controller.adb_path,
            'adb_server_port': config.adb_controller.adb_server_port,
            'console_port': config.emulator_launcher.emulator_console_port,
            'grpc_port': config.emulator_launcher.grpc_port,
        }
    except (AttributeError, TypeError):
        return {}
  
def get_device_name_from_env(env) -> str:
    """从env对象中获取device_name"""
    try:
        # 通过配置路径获取device_name
        device_name = env._coordinator._simulator._config.adb_controller.device_name
        return device_name
    except AttributeError:
        # 如果路径不存在，返回空字符串
        return ""

def uiautomator_dump_old(env) -> str:
...
def uiautomator_dump(env) -> str:
  ...
  for i in range(n_tries):
    try:
      ...
      # TODO check
      print(f'Failed to dump uiautomator: {e}')
      env._coordinator._simulator._adb_controller.execute_command(['-P', str(env._coordinator._simulator._config.adb_controller.adb_server_port), 'connect', env._coordinator._simulator._config.adb_controller.device_name], timeout=10, device_specific=False)
      time.sleep(1)
      if i == n_tries - 1:
        raise e
  return ''
