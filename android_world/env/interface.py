...
# Changes: Supporting  actuation of grounding model, UI-Venus, Qwen3.
...
from android_world.env import actuation,actuation_gd
# from android_world.env import actuation_autoglm
from android_world.env import actuation_qwen
...
class AsyncAndroidEnv(AsyncEnv):
  ...
  def update_interaction_cache(self, content: str) -> None:
    self.interaction_cache = content

  def execute_action(self, action: json_action.JSONAction, is_m3a: bool = True, actuation_type: str = '') -> None:
    ...
    elif actuation_type == 'qwen':
      actuation_qwen.execute_adb_action(
          action,
          state.ui_elements,
          self.logical_screen_size,
          self.controller,
      )
    elif actuation_type == 'gd':
      actuation_gd.execute_adb_action(
          action,
          state.ui_elements,
          self.logical_screen_size,
          self.controller,
      )
    elif actuation_type == 'qwen3':
...
# ) -> tuple[bool, str]:
    ...
    # elif 'autoglm' in actuation_type:
    #   actuation_autoglm.execute_autoglm_action(
    #       action,
    #       self.logical_screen_size,
    #       self.controller,
    #   )
    else:
      raise ValueError(f'Unsupported actuation type: {actuation_type}')
