...
def count_sent_messages(
  ...
  """统计在给定时间窗口内，符合指定号码和正文的已发短信条数。

  Args:
    messages: ADB shell content query 返回的短信记录列表。
    phone_number: 要匹配的目标号码。
    body: 要匹配的正文内容。
    current_time_ms: 当前时间（毫秒），用于计算短信是否过期。
    time_mins: 时间窗口（分钟），仅统计该时间范围内发送的短信。
...
class SimpleSMSSendSms(task_eval.TaskEval):
  ...
  # For GSATrackingBlacklistBlockAndSendSms
  def is_successful_multiple_sms(self, env: interface.AsyncEnv, expected_sms_dict: dict) -> float:
    super().is_successful(env)
    messages = self.get_sent_messages(env.controller)
    time.sleep(5)
    logging.info("During is_successful, messages: %s", messages)

    if len(messages) != len(expected_sms_dict):
      return 0.0
    ...
    # )
    android_time = self.get_android_time(env.controller)
    for phone_number, expected_body in expected_sms_dict.items():
      sms_was_sent = was_sent(
          messages,
