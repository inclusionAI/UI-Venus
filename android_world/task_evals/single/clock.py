...
class _ClockEval(task_eval.TaskEval):
  ...
  @classmethod
  def generate_random_params(cls) -> dict:
    # Every task starts October 15, 2023 @ 15:34:00.

    alarm_time_hr = random.choice([16,17,18,19,20,21,22,23])  # 0-23小时
    alarm_time_min = random.randint(0, 59)  # 0-59分钟
    alarm_time = f"{alarm_time_hr:02d}:{alarm_time_min:02d}"
...
class GSAClockStopWatchPausedatCertainTime(_ClockEval):
  """Task for checking if stop watch is paused.
  ...
  template = "Pause the stopwatch at {time_start}-{time_end}-second."
  time_gap = 2
  ...
  def is_successful(
    ...
    print("GSAClockStopWatchPausedatCertainTime")
    super().is_successful(env)
    ui_elements = env.get_state().ui_elements
    current_activity = adb_utils.get_current_activity(env.controller)[0]

    if not _is_stopwatch_paused(
            ui_elements=ui_elements,
            current_activity=current_activity,
        ):
      print("stopwatch not paused")
      return 0.0
    
    def get_ui_elements_with_retry(env, max_retries=10, target_resource_id='com.google.android.deskclock:id/stopwatch_time_text'):
      ...
      for attempt in range(max_retries):
          ...
          found = False
          for uie in ui_elements:
              # print(uie.resource_id)
              if uie.resource_id == target_resource_id:
                  print(f'found: {uie.resource_id}')
                  found = True
                  break
          ...
          if found:
              ...
              print(f"未找到 {target_resource_id}，第 {attempt + 1} 次重试...")
              if attempt < max_retries - 1:
                  time.sleep(1)  
      
      print(f"重试 {max_retries} 次后仍未找到目标元素")
      return None
    
   
    ui_elements = get_ui_elements_with_retry(env)
    for element in ui_elements:
      if element.resource_id == 'com.google.android.deskclock:id/stopwatch_time_text':
        print("paused time: ", int(element.text))
        print(f"self.params['time_start']: {self.params['time_start']}")
        print(f"self.params['time_end']: {self.params['time_end']}")

        if ( self.params['time_start'] <= int(element.text) <= self.params['time_end']):
          return 1
        else:
          return 0
