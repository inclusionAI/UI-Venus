...
# Changes: Implementing tasks in VenusBench such as GUIBrowsingBrowserRandomButtons2.
...
from android_world.utils import generate_html_v7 as generate_html
...
class GUIBrowsingBrowserRandomButtons1(task_eval.TaskEval):
  ...
  def generate_random_params(cls) -> dict[str, Any]:
    # return {'browser_task_seed': random.randint(0, 2**32 - 1)}
    html_content, icon_to_feature_map = generate_html.generate_random_html_for_agent(button_type=cls.button_type)
    # cls.HTML = html_content
    return {'HTML': html_content, 'icon_to_feature_map':icon_to_feature_map}
  
  @property
  def goal(self) -> str:

    icon_to_feature_map = self.params['icon_to_feature_map']
    for m in icon_to_feature_map:
      if m['feature_id'] == self.button_type:
        prompt = m['prompt']
    return (
        self.preamble
        + ' You can only finish the following task in the task.html.'
        + prompt
    )


class GUIBrowsingBrowserRandomButtons2(GUIBrowsingBrowserRandomButtons1):
  button_type="notepad"
  @property
  def goal(self) -> str:
    icon_to_feature_map = self.params['icon_to_feature_map']
    for m in icon_to_feature_map:
      if m['feature_id'] == 'notepad':
        prompt = m['prompt']
    return (
        self.preamble
        + ' You can only finish the following task in the task.html.'
...
class GUIBrowsingBrowserRandomButtons2CHS(GUIBrowsingBrowserRandomButtons1):
  button_type="notepad"
  ...
  def goal(self) -> str:
    ...
    for m in icon_to_feature_map:
      if m['feature_id'] == 'notepad':
        prompt = m['prompt']
        param =  prompt.split()[-1]
    
        return (
          f'在文件管理器中打开“Downloads”目录下的task.html；'
          '出现选择应用提示时，用 Chrome 打开。后续任务只能在 task.html 内完成。'
          f'请准确地把以下内容输入到页面里的临时记事本中：{param}'
                )

class GUIBrowsingBrowserRandomButtons2Variation(GUIBrowsingBrowserRandomButtons1):
  button_type="notepad"
  ...
  def goal(self) -> str:
    ...
    for m in icon_to_feature_map:
      if m['feature_id'] == 'notepad':
        ...
        return f'Open Downloads/task.html in file manager; when prompted, open it with Chrome. The next steps can only be done inside task.html. Please type the following text precisely into the Temporary Notepad: {param}'
    # return (
    #     self.preamble
    #     + ' You can only finish the following task in the task.html.'
    #     + prompt
...
class GUIBrowsingBrowserRandomButtons3(GUIBrowsingBrowserRandomButtons1):
  button_type="color_changer"

  @property
  def goal(self) -> str:
    icon_to_feature_map = self.params['icon_to_feature_map']
    for m in icon_to_feature_map:
      if m['feature_id'] == 'color_changer':
        prompt = "Use some button shown in this html to change the background color."
    return (
        self.preamble
        + ' You can only finish the following task in the task.html.'
        + prompt
...
class GUIBrowsingBrowserRandomButtons4(GUIBrowsingBrowserRandomButtons1):
  button_type="random_number"

  @property
  def goal(self) -> str:
    icon_to_feature_map = self.params['icon_to_feature_map']
    for m in icon_to_feature_map:
      if m['feature_id'] == 'random_number':
        prompt = "Use some button shown in this html to generate a random number."
    return (
        self.preamble
        + ' You can only finish the following task in the task.html.'
        + prompt
...
class GUIBrowsingBrowserRandomButtons5(GUIBrowsingBrowserRandomButtons1):
  button_type="calculator"

  @property
  def goal(self) -> str:
    icon_to_feature_map = self.params['icon_to_feature_map']
    for m in icon_to_feature_map:
      if m['feature_id'] == 'calculator':
        prompt = m['prompt']
    return (
        self.preamble
        + ' You can only finish the following task in the task.html.'
