...
# Changes: Implementing tasks in VenusBench using recipe app.
...
class MultiroundRecipeAddMultipleRecipesReverse(sqlite_validators.AddMultipleRows, _RecipeApp):
  """Task to delete multiple recipes in Broccoli Recipe App."""

  complexity = 3
  n_rows = 4
  n_rows_noise = 10
  addrecipes_1stround_verified = False
  ...
  def goal(self) -> str:
    if self.round == 0:
        ...
        return "Undo all recipe addtion actions you performed."


  def is_successful_first_round(self, env: interface.AsyncEnv) -> float:
    ...
    row_addition_successful = self.validate_addition_integrity(
        self.before, after, self.params['target_first_two_rows']
    )
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    if row_addition_successful and self.addrecipes_1stround_verified:
      return 1
    else:
      return 0
  ...
  def check_status(self, env: interface.AsyncEnv):
    if self.addrecipes_1stround_verified == False:
      task1_success = self.is_successful_first_round(env)
      if task1_success:
        self.addrecipes_1stround_verified = task1_success
        self.start_step_2ndround = self.current_step + 3
    if self.addrecipes_1stround_verified and self.start_step_2ndround == self.current_step:
      self.round = 1
  ...
  def generate_random_params(cls) -> dict[str, Any]:
    ...
    return {
        ...
        "target_first_two_rows": target_rows[:2],
        sqlite_validators.NOISE_ROW_OBJECTS: noise_rows,
        _TEXT_REPRESENTATION_TYPE: random.choice(['csv', 'text_block']),
    }
...
class MultiRoundRecipe1AddImageRecipes2AddMarkor3DeleteImage(_RecipeAddMultipleRecipes):
  """Task to add multiple recipes from an image file to Broccoli Recipe App."""
  ...
  def goal(self) -> str:
    if self.round == 0:
        ...
        return (
          f'Delete the recipes read from images.'
        )
    
  def initialize_task(self, env: interface.AsyncEnv):
    super().initialize_task(env)

    # 1st round: add recipe from image
    user_data_generation.clear_device_storage(env)
    data = _get_rows_as_text(
        self.params[sqlite_validators.ROW_OBJECTS], 'text_block', wrap_width=60
    )
    ...
    # 2nd round: add recipe from markor
    rows = (
        self.params['row_objects2']
        + self.params[sqlite_validators.NOISE_ROW_OBJECTS]
    )
    random.shuffle(rows)
    file_utils.clear_directory(device_constants.MARKOR_DATA, env.controller)
  ...
  def tear_down(self, env: interface.AsyncEnv):
    super().tear_down(env)
    # 1st round from image
    user_data_generation.clear_device_storage(env)
    # 2nd round from markor
    file_utils.clear_directory(device_constants.MARKOR_DATA, env.controller)
  ...
  def validate_deletion_integrity(
    ...
    """Validates the integrity of the recipe deletion."""
    return sqlite_validators.validate_rows_removal_integrity(
        before, after, [r.recipeId for r in self.params[sqlite_validators.ROW_OBJECTS]], self.db_key
    )
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    if self.round == 0:
      ...
      after = self.list_rows(env)
      row_addition_successful = self.validate_addition_integrity(
          self.before, after, self.params['row_objects2']
      )
      if row_addition_successful:
        self.before = self.list_rows(env)
        return 1.0 
      ...
      recipes_to_delete = self.params[sqlite_validators.ROW_OBJECTS]
      if len(self.before) != len(after) + len(recipes_to_delete):
        return 0.0
      
      titles_recipes_to_delete = [r.title for r in recipes_to_delete]
      # Check that no new unexpected rows have been added
      for row in after:
        if row not in self.before:
          return 0
        if row.title in titles_recipes_to_delete:
          return 0
      return 1
  ...
  def generate_random_params(cls) -> dict[str, Any]:
    ...
    target_rows_titles = {t.title for t in target_rows}  # 先提取所有 title


    # 2nd
    """Generate random parameters for an add recipe task."""
    prep_time = random.choice(_PREP_TIME_OPTIONS)
    target_rows2 = sqlite_schema_utils.get_random_items(
        cls.n_rows,
        _generate_random_recipe,
        replacement=False,
        filter_fn=lambda r: r.preparationTime == prep_time and 
        r.title not in target_rows_titles,
    )
    
    all_target_rows = target_rows + target_rows2
    noise_rows = sqlite_schema_utils.get_random_items(
        cls.n_rows_noise,
        _generate_random_recipe,
        replacement=False,
        ...
        filter_fn=lambda r: (
            r.preparationTime != prep_time
            and 
            all(r.title != t.title for t in all_target_rows)
        ),
    )
...
class FuncAssistRecipeExplainOneFunctionality1(_RecipeAddMultipleRecipes):
  ...
  def goal(self) -> str:
    return (
        'I want to add a new recipe in the Broccoli. '
        'What content I can customize in this new recipe? ' \
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    gt_answer = "You can customize the details of your recipe, including the title, ingredients, description, source, servings, time, ingredients and directions. Optionally, you can add a photo of the dish by tapping on the camera icon."
    ...
    try:
      ...
      logging.warning(
          'Error during verification for FuncAssistRecipeExplainOneFunctionality1: %s', e
      )
      return 0.0

class FuncAssistVagueRecipeExplainOneFunctionality(_RecipeAddMultipleRecipes):
  """Task to delete a single recipe in Broccoli Recipe App."""
  ...
  def goal(self) -> str:
    return (
        'I have a recipe I want to record, '
        'but I do not know how to do it. Please tell me how to do it.'
    )
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    goal = self.goal
    prediction = env.interaction_cache
    gt_answer = "You can follow these steps: 1. Open the Broccoli app on your device. 2. Tap on the '+' button located at the bottom right corner of the screen to create a new recipe. 3. Fill in the details of your recipe, including the title, ingredients, description, and so on. 4. Optionally, you can add a photo of the dish by tapping on the camera icon. 5. Once you have entered all the necessary information, tap on the 'Save' button to save your new recipe."
    ...
    try:
      ...
      logging.warning(
          'Error during verification for FuncAssistVagueRecipeExplainOneFunctionality: %s', e
      )
      return 0.0

# 
class FuncAssistRecipeLocateOneFunctionality1(_RecipeAddMultipleRecipes):
  """Task to delete a single recipe in Broccoli Recipe App."""
  ...
  def goal(self) -> str:
    return (
        'I want to add a new recipe in the Broccoli. '
        'Locate the infeface where I can set the content of the new recipe and show me.'
    )
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    goal = self.goal
    task_key = 'FuncAssistRecipeLocateOneFunctionality1'
    ...
    try:
      ...
      logging.warning(
          'Error during verification for FuncAssistRecipeLocateOneFunctionality1: %s', e
      )
      return 0.0


class RefusalRecipeAddSingleRecipeConflict1(_RecipeAddMultipleRecipes):
  """Task to delete a single recipe in Broccoli Recipe App."""
  ...
  answer = (
    "Sorry, I can't add the recipe. "
    "The Quality and Flavor fields aren't available when adding a new recipe."
    )
  ...
  def goal(self) -> str:
    ...
    return f'Add the following recipes into the Broccoli app:\n{text_repr}\nFill the Quality field with "14 kg", and the Flavor field with "spicy".'

  def is_successful(self, env: interface.AsyncEnv) -> float:
    after = self.list_rows(env)
    if len(self.before) != len(after):
      return 0.0
    ply = VerifyPolicy({})
    result = ply.verify_textual_prediction(
        goal=self.goal,
...
def _generate_random_recipe_with_setting(ingredient) -> sqlite_schema_utils.Recipe:
  """Generates a random recipe."""
  ...
  return dataclasses.replace(
      ...
      preparationTime=random.choice(_PREP_TIME_OPTIONS),
      directions=f'{recipe.directions} {random.choice(directions_additions)}',
      ingredients=random.choice(ingredient_descriptors) + f', {ingredient}',
  )

class RefusalRecipeDeleteMultipleRecipesWithConstraintAmbigious1(_RecipeDeleteMultipleRecipes):
  """Delete multiple recipes in Broccoli Recipe App based on ingredient."""
  ...
  answer = ('No recipe contains the ingredient you told in its Directions, so no recipes were removed.')
  ...
  def is_successful(self, env: interface.AsyncEnv) -> float:
    ...
    ans = self.answer.format(self.params['ingredient'])
    result = ply.verify_textual_prediction(
            goal=self.goal,
            prediction=env.interaction_cache,
            gt_answer=ans,
  ...
  def generate_random_params(cls) -> dict[str, Any]:
    ...
    while n_rows > 0:
      try:
        # add ambigious info in 
        targets = sqlite_schema_utils.get_random_items(
            n_rows,
            lambda: _generate_random_recipe_with_setting(ingredient),
            replacement=False,
            filter_fn=lambda r: ingredient not in r.directions.lower(),
        )
