...
def generate_random_html_for_agent(button_type=None):
    """
    ...
    Args:
        button_type (str or list): Specify which feature(s) should show "Success!" logic.
                                   Can be a single feature_id string or a list of feature_ids.
                                   If None, all features will show "Success!" (default behavior).
                                   Examples: 'calculator', ['calculator', 'notepad'], etc.
    ...
    # Convert button_type to list for easier checking
    if button_type is None:
        allowed_types = None  # All types allowed
    elif isinstance(button_type, str):
        allowed_types = [button_type]
    else:
        allowed_types = list(button_type)

    # Helper function to check if success message should be enabled
    def should_enable_success(feature_id):
        if allowed_types is None:
            return True
        return feature_id in allowed_types
    ...
    features = [
        {
            ...
            'html_template': textwrap.dedent("""
                ...
                {success_msg}
            """),
            'js_init_template': 'document.getElementById("calc-display").innerText = "0"; {success_init}',
            'js_logic_template': """
                case '=':
                    try {{
        ...
        },
        {
            ...
            'html_template': textwrap.dedent("""
                ...
                {success_msg}
            """),
            'js_init_template': "document.getElementById('search-input').value = ''; {success_init}",
            'js_logic_template': """
        ...
        },
        {
            ...
            'html_template': textwrap.dedent("""
                ...
                {success_msg}
            """),
            'js_init_template': 'document.getElementById("notepad-area").value = ""; {success_init}',
            'js_logic_template': """
        ...
        {'id': 'location', 'name': 'Get My Location', 
         'html_template': '<p id="location-status">Click the button to get your current coordinates.</p><button onclick="get_location()">Get Location</button><div id="location-result" style="margin-top:10px; font-weight:bold; white-space: pre-wrap;"></div>{success_msg}', 
         'js_init_template': 'document.getElementById("location-status").innerText = "Click the button to get your current coordinates."; document.getElementById("location-result").innerText = ""; {success_init}'},
        {'id': 'color_changer', 'name': 'Background Changer', 
         'html_template': '<p>Click the button to change the background color.</p><button onclick="change_bg_color()">Random Color</button>{success_msg}', 
         'js_init_template': '{success_init}'},
        ...
        {'id': 'random_number', 'name': 'Random Number Generator', 
         'html_template': '<p>Generate a random number between 1 and 100.</p><button onclick="generate_random_number()">Generate</button><p id="random-number-display" style="font-size: 2em; text-align:center; font-weight:bold; margin-top:10px;"></p>{success_msg}', 
         'js_init_template': 'document.getElementById("random-number-display").innerText = ""; {success_init}'}
    ]
    ...
    for i in range(10):
        ...
        # Check if this feature should have success message
        enable_success = should_enable_success(feature_id)
        ...
        mapping_entry = {
            ...
            'has_success_validation': enable_success
        }

        # Generate success message HTML or empty string
        success_msg_html = '<p id="{}-success-msg" class="success-message">Success!</p>'.format(feature_id) if enable_success else ''
        success_init_js = 'document.getElementById("{}-success-msg").style.display="none";'.format(feature_id) if enable_success else ''
        ...
        if feature_id == 'notepad':
            ...
            success_check_js = """if (current === target) {{
                ...
                }}""" if enable_success else ""
            
            feature_info['html'] = feature_info['html_template'].format(success_msg=success_msg_html)
            extra_js_logic += feature_info['js_logic_template'].format(target_str=target_str, success_check=success_check_js)
            feature_info['js_init'] = feature_info['js_init_template'].format(success_init=success_init_js)
            ...
            success_check_js = """if (result == {}) {{
                            document.getElementById('calculator-success-msg').style.display = 'block';
                        }}""".format(target_number) if enable_success else ""
            
            feature_info['html'] = feature_info['html_template'].format(buttons=calc_buttons_html, success_msg=success_msg_html)
            calculator_js_logic = feature_info['js_logic_template'].format(success_check=success_check_js)
            feature_info['js_init'] = feature_info['js_init_template'].format(success_init=success_init_js)
            mapping_entry['prompt'] = prompt
            mapping_entry['target_value'] = target_number
            
        elif feature_id == 'location':
            feature_info['html'] = feature_info['html_template'].format(success_msg=success_msg_html)
            feature_info['js_init'] = feature_info['js_init_template'].format(success_init=success_init_js)
            
        elif feature_id == 'color_changer':
            feature_info['html'] = feature_info['html_template'].format(success_msg=success_msg_html)
            feature_info['js_init'] = feature_info['js_init_template'].format(success_init=success_init_js)
            
        elif feature_id == 'random_number':
            feature_info['html'] = feature_info['html_template'].format(success_msg=success_msg_html)
            feature_info['js_init'] = feature_info['js_init_template'].format(success_init=success_init_js)
    ...
    # Update get_location function to conditionally show success
    location_success_js = """document.getElementById('location-success-msg').style.display = 'block';""" if should_enable_success('location') else ""
    
    # Update change_bg_color function to conditionally show success
    color_success_js = """document.getElementById('color_changer-success-msg').style.display = 'block';""" if should_enable_success('color_changer') else ""
    # color_success_js = """document.getElementById('color_changer-success-msg').style.display = 'block';""" if should_enable_success('color_changer') else ""
    
    # Update generate_random_number function to conditionally show success
    random_num_success_js = """document.getElementById('random_number-success-msg').style.display = 'block';""" if should_enable_success('random_number') else ""
...
if __name__ == "__main__":
    html, mapping = generate_random_html_for_agent(button_type="notepad")
    with open("agent_v7_notepad.html", "w", encoding="utf-8") as f:
        f.write(html)
    print("agent_v7.html 已生成，仅 notepad 任务完成后会显示 Success!")
