...
class InfoPool:
    ...
    # raw responses
    raw_response_list: list[str] = field(default_factory=list)
...
class Manager(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        if info_pool.plan == "":
            ...
            if info_pool.additional_knowledge_manager[0] != "":
                if len(info_pool.additional_knowledge_manager) > 1:
                    prompt += f"{info_pool.additional_knowledge_manager}\n\n"
                else:
                    prompt += f"{info_pool.additional_knowledge_manager[0]}\n\n"
            else:
...
from android_world.agents.new_json_action import *
...
ATOMIC_ACTION_SIGNITURES_noxml = {
    ...
    TYPE: {
        "arguments": ["text"],
        "description": lambda info: "Type text into current activated input box or text field. There may be a cursor in the activated input box. If not, click the input box to confirm again. Please make sure the correct input box has been activated before typing. Usage Example: {\"action\": \"type\", \"text\": \"the text you want to type\"}"
    },
    SYSTEM_BUTTON: {
        "arguments": ["button"],
...
class Executor(BaseAgent):
    ...
    def get_prompt(self, info_pool: InfoPool) -> str:
        ...
        if info_pool.additional_knowledge_executor[0] != "":
            prompt += "### Guidelines ###\n"
            if len(info_pool.additional_knowledge_executor) > 1:
                prompt += f"{info_pool.additional_knowledge_executor}\n"
            else:
                prompt += f"{info_pool.additional_knowledge_executor[0]}\n"
