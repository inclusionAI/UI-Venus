    ...
    if action_type == json_action.INPUT_TEXT:
      ...
      app_name = dummy_action_translated['arguments']['text']
        ...
        if status in ["failure", "failed"]:
            goal_status = "failure"
        elif status in ["infeasible", "cannot", "abort"]:
            goal_status = "infeasible"
        else:
            goal_status = "task_complete"

    elif action_type == 'system_button':
        if arguments['button'] == 'Back' or arguments['button'] == 'back':
            action_type = json_action.NAVIGATE_BACK
        elif arguments['button'] == 'Home' or arguments['button'] == 'home':
            action_type = json_action.NAVIGATE_HOME
        elif arguments['button'] == 'Enter' or arguments['button'] == 'enter':
